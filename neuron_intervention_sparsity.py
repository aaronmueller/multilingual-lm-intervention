...
# get global list 
def get_all_contrib(model, templates, tokenizer, attractor, out_dir='sparsity'):
  df = get_intervention_results(model, templates, tokenizer, attractor=attractor)
  gc.collect()
  ...
  # CHANGE HERE
  layer_list = [i-1 for i in layer_list]
  ...
  pickle.dump(marg_contrib, open(out_dir + "/marg_contrib_" + attractor + "_" + model_type + ".pickle", "wb" ))
  return layer_list, neuron_list

def get_intervention_results(model, templates, tokenizer, DEVICE='cuda', attractor='none', seed=12, examples=200,
...
def top_k(model, model_type, tokenizer, attractor, templates, layer_list, neuron_list, k=50, out_dir='sparsity'):
  odd_abs_list = []
  ...
  for i in range(k):
    ...
    odd_abs_list.append(df['odds_ratio'].mean()-1)
    # print(odd_abs_list)
    pickle.dump(odd_abs_list, open(out_dir + "/topk_" + model_type + '_' + attractor + ".pickle", "wb" ))
...
if __name__ == '__main__':
    ...
    if args.algo == 'topk':
        ...
        layer_list, neuron_list = get_all_contrib(model, templates, tokenizer, attractor, out_dir)
        if layer == -1:
            top_k(model, model_type, tokenizer, attractor, templates, layer_list, neuron_list, k, out_dir)
            res = get_neuron_intervention_results(model, templates, tokenizer, layer_list, neuron_list)
            pickle.dump([res], open(out_dir + "/topk_" + model_type + '_' + attractor + "_all.pickle", "wb" ))
        elif layer != -1:
            top_k_by_layer(model, model_type, tokenizer, attractor, templates, layer, layer_list, neuron_list, k, out_dir)
