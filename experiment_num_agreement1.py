...
class Model():
    ...
    def get_probabilities_for_examples_multitoken(self, context, candidates):
       ...
       # TODO: Combine into single batch
        ...
        return np.array([mean_probs]).transpose()
    
    def get_probabilities_for_examples_multitoken_noun(self, context, candidates):
        if len(context > 1):
            prob_base = self.get_probabilities_for_examples_multitoken(context[:,:-1],[[int(context[0,-1])]])
            probs = self.get_probabilities_for_examples_multitoken(context, candidates) / prob_base
        else:
            probs = self.get_probabilities_for_examples_multitoken(context, candidates)
    ...
    def neuron_intervention(self,
        ...
        for hndle in handle_list:
            hndle.remove()
        print('neuron_intervention prob: ', len(new_probabilities))
    ...
    def neuron_intervention_single_experiment_multitoken(self, intervention, effect, layers_to_adj, heads_to_adj, search):
        """
        Run one full neuron intervention experiment
        measuring indirect or direct effect.
        """
