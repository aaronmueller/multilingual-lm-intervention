""" A copy of transformers/modeling_xglm.py from the Huggingface
...
class XGLMFeedForward(nn.Module):
...
class XGLMDecoderLayer(nn.Module):
    def __init__(self, config: XGLMConfig):
        ...
        self.ff = XGLMFeedForward(config)
...
class XGLMPreTrainedModel(PreTrainedModel):
    ...
    def _load_state_dict_into_model(
        ...
        for key in state_dict.keys():
            ...
            if "layers." in key and "self_attn" not in key:
                new_key = key.split(".")
                new_key.insert(3, "ff")
